hid_codes = {
    ...
    # special codes for power glove purposes (not exaustive yet)
    'MOVE_MOUSE_X_L' : 0xfa,
    'MOVE_MOUSE_X_R' : 0xfb,
    'MOVE_MOUSE_Y_U' : 0xfc,
    'MOVE_MOUSE_Y_D' : 0xfd,
...
def get_code(key):
    return int(hid_codes[key])

def key_to_byte(key):
    packet = bytearray()
    if type(key) == str:
        packet.append(get_code(key))
    elif type(key) == list:
        for k in key:
            packet.append(get_code(k))
    else:
        print("input must be type str or list!")
        raise(ValueError)
    return packet
