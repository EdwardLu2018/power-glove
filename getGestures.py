...
def getSwipeInfo(pose, data, count, tempcount, ser1, clf, right):
    ...
    swipeType = detectSwipe(orientation, velx_data, vely_data, velz_data, ser1, right)
    swipevert = (detectSwipe(orientation, velx_data, vely_data, velz_data, ser1, right) != None)
    ...
    if (tempcount < count - 10):
        ...
        if (swipeside or swipevert) and not (swipeside and swipevert) and (pose == clf.NEUTRAL or pose == clf.OPEN):
            if (swipevert):
                print(detectSwipe(orientation, velx_data, vely_data, velz_data, ser1, right))
                swipeType = detectSwipe(orientation, velx_data, vely_data, velz_data, ser1, right)
            else:
